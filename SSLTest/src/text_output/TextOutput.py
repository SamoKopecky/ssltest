...
class TextOutput:
    ...
    def print_address(self):
        ...
        """
        self.print_title(2, f"Scan for {self.address.url}:{self.address.port}", self.address_filler)
    ...
    def print_parameters(self, data, indent):
        ...
        for key, value in data.items():
            ...
            if type(value) != dict:
                ...
                continue
            printn(f"{self.get_color_for_value(next(iter(value.values())))} -- {next(iter(value.keys()))}")
