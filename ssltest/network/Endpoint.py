...
class Endpoint:
    ...
    def scan_endpoint(self):
        ...
        if 'SSL' in protocol:
            ...
            with SecureSafeSocket(self.sock_addr, protocol, True, 'tlsv1.n_scan') as sock:
                sock.connect()
    ...
    def choose_protocol(self):
        ...
        """
        tls_protocols = list(
            filter(lambda p: 'TLS' in p, self.supported_protocols))
    ...
    def get_cipher_suite_and_protocol(sock):
        ...
        if '-' in cipher_suite:
            ...
            cipher_suite = convert_cipher_suite(
                cipher_suite, 'OpenSSL', 'IANA')
